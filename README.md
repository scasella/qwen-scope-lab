...
- [Distilling polite truth-holding (write-up)](https://scasella.github.io/qwen-scope-lab/writeups/polite-truth-holding-distillation.html) — the v0.6→v1.0 paper: a class-balanced 9B-teacher corpus gives a 4B both truth-holding under pressure and calibrated uncertainty; replicated 6/6 seeds, the fix is the mixture not the count, gpt-5.5-corroborated.
...
- [`docs/experiments/BEHAVIOR_READOUT_C05.md`](docs/experiments/BEHAVIOR_READOUT_C05.md) — C05: the manifold-vs-linear faithfulness verdict flips on multi-token concepts under a full-string read-out (and on none of the single-token controls); includes the corrected full census ledger (4/6, with the composition change that matters).
- [Walking a model through six emotions, in order (write-up)](https://scasella.github.io/qwen-scope-lab/writeups/emotion-manifold-centroid-routing.html) — manifold steering induces every intermediate emotion in sequence on the arousal axis (agreement 1.00 vs linear 0.33, vs random 0.17, vs shuffled-order 0.83); the valence and fear lines fail with legible mechanisms. Method: [`docs/experiments/EMOTION_MANIFOLD.md`](docs/experiments/EMOTION_MANIFOLD.md).
- [The manifold atlas — twelve more clean concept geometries (write-up)](https://scasella.github.io/qwen-scope-lab/writeups/manifold-atlas-3d.html) — interactive 3D plots of the real fitted geometry. A census of 20 new concepts found 12 clean manifolds (0 diffuse); the honest companion finding is that only 2 of 11 tested also *route* — and a recovery diagnostic with softer metrics shows the rest are graded-controllable by a plain *linear* steer, not the manifold (the manifold-specific advantage stays singular to one concept). Clean geometry ≠ clean control. Method: [`docs/experiments/MANIFOLD_ATLAS_CENSUS.md`](docs/experiments/MANIFOLD_ATLAS_CENSUS.md).
- [The rings a straight line can't follow (interactive demo)](https://scasella.github.io/qwen-scope-lab/writeups/cyclic-ring-steering.html) — the cyclic exception, walkable, on **net-new rings of our own**: months, zodiac, color wheel, Chinese zodiac, and card ranks all beat linear steering (a sixth, the compass, passes only weakly and is recorded in the strip, not showcased), with six honest fails each catching a named failure mode and days-of-week kept as the credited Goodfire baseline. Companion experience: [The steering field](https://scasella.github.io/qwen-scope-lab/writeups/steering-field.html) — the same measured walks as a particle-physics race: a spring-steered comet rides the fitted ring while the linear comet cuts the interior, and every waypoint burst is the real token + probability the model produced. The headline walk crosses the **King→Ace seam** (Jack→Three: Queen, King, Ace, Two induced in order, 1.00 vs the chord's 0.50) — in activation space Ace's nearest neighbours are Jack, King, Queen, so the ace-high cluster stitches a wrap that text rarely states. Data: `reports/manifold_census/{cyclic_atlas,manifold_zoo,card_seam}.json`.
