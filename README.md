...
- [The ring a straight line can't follow (interactive demo)](docs/writeups/cyclic-ring-steering.html) — the cyclic exception, walkable: scrub a teal walker around the actual fitted days-of-week ring and watch the model name every weekday in order (routing 1.00), while the linear chord's walker snaps Mon→Fri across the ring's interior (0.40). The one demonstrated lane where the manifold is non-dominated. Data: `reports/manifold_census/cyclic.json`.
