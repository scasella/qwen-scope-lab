...
- [The rings a straight line can't follow (interactive demo)](docs/writeups/cyclic-ring-steering.html) — the cyclic exception, walkable, on **four net-new rings of our own**: the months of the year (Jan→Jul routes 1.00 vs the chord's 0.29 — the chord is a *diameter*), the zodiac (1.00 vs 0.29), the color wheel (1.00 vs 0.40), and the compass (0.60, cardinals only), plus two honest fails (time-of-day caught by the shuffled control; moon phases steer-resistant) and days-of-week kept as the credited Goodfire baseline. Scrub a walker around each real fitted ring and watch the model name every value in order. Data: `reports/manifold_census/cyclic_atlas.json`.
