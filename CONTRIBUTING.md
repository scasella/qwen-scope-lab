...
On an Apple Silicon Mac you can also test against the **real** model on-device: `pip install -e ".[mlx]"`, then `python serve_web.py --mlx` (or `--mlx-base` for the base-model pairing; see `docs/MLX.md`). The base install is deliberately slim — heavy/cloud deps live in opt-in extras (`mlx`, `cuda`, `modal`, `gradio`, `all`).
