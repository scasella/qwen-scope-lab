...
image = (
    ...
    .add_local_dir("qwen_scope_lab", remote_path="/root/qwen_scope_lab")
...
def _web_parity(config_path: str, layer: int, max_new_tokens: int) -> dict:
    ...
    Lab web layer (web_gui) reaches parity with the dev backend on real Qwen.
    """
...
def _residual_manifold_sweep(config_path: str, layers: list[int], k_pca: int = 8, seed: int = 7) -> dict:
    ...
    from qwen_scope_lab.hooks import register_capture_hook
    from qwen_scope_lab.service import SteeringService
...
def _manifold_atlas(config_path: str, layers: list[int]) -> dict:
    ...
    from qwen_scope_lab.concept_presets import ATLAS_CONCEPTS
    from qwen_scope_lab.hooks import register_capture_hook
    from qwen_scope_lab.service import SteeringService
...
def _emotion_safety_demo(config_path: str, layer: int = 12, emotions=("affection", "anger", "fear"),
    ...
    from qwen_scope_lab import emotion_sets as ES
    from qwen_scope_lab import judge as _judge
    from qwen_scope_lab.service import SteeringService
...
def _bench_smoke(config_path: str, layer: int, max_new_tokens: int, prompt_count: int, recipe_name: str) -> dict:
    ...
    from qwen_scope_lab.benchmark import ServiceGenerationBackend, attach_benchmark_to_recipe, run_benchmark, save_benchmark_result
...
@modal.asgi_app()
def web_gui():
    # The Lab: same SteeringService backend as gradio_gui, served through the
