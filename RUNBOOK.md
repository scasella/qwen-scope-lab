...
**Geometry-investigation harness** — ARCHIVED. The dead-end exploration that led to manifold steering (`latent_map_2b/_27b` decoder-cosine layout — DEAD; `coact_map_2b` co-activation layout — DEAD; `manifold_probe_2b` Ising conditional-coupling — DEAD; `residual_manifold_2b`; `manifold_vs_linear_probe_2b`) has been moved out of `modal_app.py` into `archive/research_probes.py`. It is preserved for provenance but is no longer runnable as-is; see `archive/README.md` for the full negative-result writeup.
